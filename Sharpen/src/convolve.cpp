...
/*
    for(int x = 0; x < augmented.rows; ++x){
        float* ptr = augmented.ptr<float>(x);
        for(int y = 0; y < augmented.cols; ++y){
            if(x == 0 || y == 0 || x == augmented.rows - 1 || y == augmented.cols - 1) ptr[y] = 0.0;
            std::cout << ptr[y] << '\t';
        }
        std::cout << '\n';
    }
