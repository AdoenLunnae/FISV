...
void fsiv_lbp_hist(const cv::Mat& lbp, cv::Mat& lbp_hist, bool normalize = true, int nbins = 256);

//! Computes a LBP descriptor (concatenation of cells)
//! \param ncells [in] [rows x cols] E.g. {6,4}
void fsiv_lbp_desc(const cv::Mat& image, cv::Mat& lbp_desc, const int* ncells, bool normalize = true, bool asrows = true, int nbins = 256);
...
//! Transforms an LBP matrix into the uniform equivalent
