...
int compute_lbp_from_list(const std::vector<std::string>& lfiles, cv::Mat& train_lbp, const int* ncells, const bool normalize, const bool asrows, const bool withmirror)
{
    for (int fix = 0; fix < lfiles.size(); fix++) {
        ...
        cv::Mat descriptor;
        fsiv_lbp_desc(img, descriptor, ncells, normalize, asrows, 59);

        if (fix == 0)
            train_lbp = descriptor.clone();
        else {
            if (asrows)
                cv::vconcat(train_lbp, descriptor, train_lbp);

            else
                cv::hconcat(train_lbp, descriptor, train_lbp);
        }
    }
    return asrows ? train_lbp.rows : train_lbp.cols;
}
...
compute_intersection_over_union(const cv::Rect& boxA, const cv::Rect& boxB)
{
    ...
    return static_cast<double>(interArea) / static_cast<double>(boxA.area() + boxB.area() - interArea);
}
